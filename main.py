...
class VoiceMonitor:
    ...
    def on_received_start_speaking(self, data, addr):
        print("\033[35m", f"{datetime.now()} receive start_speaking", "\033[0m", sep="")
        self._is_speaking = True
        self._timeout = 600

    def on_received_end_speaking(self, data, addr):
        print("\033[35m", f"{datetime.now()} receive end_speaking", "\033[0m", sep="")
        self._is_speaking = False
...
def main(args):
    ...
    if args.mode == 'active':
        while len(device_monitor._device_list) == 0:
            time.sleep(2)
    ...
    while True:
        ...
        if len(device_monitor._device_list) == 0:
            ...
            if status == 'wait_for_prompt':
                ...
                print("\033[35m", f"{datetime.now()} send start_speaking", "\033[0m", sep="")
                synthesizer.play_stram()
                sender.send_dict(
                    message_type='end_speaking',
                    content={}
                )
                print("\033[35m", f"{datetime.now()} send end_speaking", "\033[0m", sep="")
